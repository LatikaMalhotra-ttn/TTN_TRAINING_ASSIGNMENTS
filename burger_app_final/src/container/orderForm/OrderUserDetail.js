...
class UserDetail extends Component{
    ...
    provideElements=(elementObj)=>{
            switch(elementObj.elementType){
                ...
                case "dropdown":
                    return (
                        <select key={shortid.generate()} 
                            className="custom-select" 
                            onChange={(e)=> this.inputChangeHandler(e,elementObj)} 
                            required={true}>
                            {
                                elementObj.options.map((option)=>( option==="Payment Method"? 
                                <option key={shortid.generate()} value={option}>{option}</option>
                                : <option key={shortid.generate()} style={{color:"black"}} 
                                    value={option}>{option}
                                  </option>))
                            }
    ...
    render(){
        return(
            <form className={classes.UserDetailForm } onSubmit={this.formSubmit}>
                <i style={{cursor:"pointer",color:"red",textAlign:"right"}} 
                    data-toggle="tooltip" 
                    title="Close" 
                    onClick={this.close} 
